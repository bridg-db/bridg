...
import { parseModelNamesFromSchema } from '../generator/generate';
import { generateSchemaRelations } from '../generator/server/serverTypes.generate';
import { readFileAsString } from '../utils/file.util';

const schema1 = readFileAsString('src/__tests__/__fixtures__/schema1.prisma');
const schema2 = readFileAsString('src/__tests__/__fixtures__/schema2.prisma');
const schema3 = readFileAsString('src/__tests__/__fixtures__/schema3.prisma');
const schema4 = readFileAsString('src/__tests__/__fixtures__/schema4.prisma');

it('parses model names', () => {
  expect(parseModelNamesFromSchema(schema1)).toStrictEqual(['User', 'Blog']);
  expect(parseModelNamesFromSchema(schema2)).toStrictEqual([
    'User',
    'Blog',
    'Post',
  ]);
  expect(parseModelNamesFromSchema(strip(schema3))).toStrictEqual([
    'User',
    'Blog',
    'Post',
    'Comment',
